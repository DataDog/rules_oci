...
func GenerateBuildFilesHandler(handler images.HandlerFunc, layoutRoot string, provider content.Provider) images.HandlerFunc {
	...
	ldBlob := rule.NewLoad("@com_datadoghq_cnab_tools//rules/oci:blob.bzl")
	ldBlob.Add("oci_blob")

	ldManifest := rule.NewLoad("@com_datadoghq_cnab_tools//rules/oci:manifests.bzl")
	...
	ldLayout := rule.NewLoad("@com_datadoghq_cnab_tools//rules/oci:layout.bzl")
